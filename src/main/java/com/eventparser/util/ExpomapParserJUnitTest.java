 ...
 */
public class ExpomapParserJUnitTest {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("\n[DEBUG_LOG] Пробуем специфичные селекторы для expomap.ru:");
            analyzeSelectors(doc, ".expo-list-item, .expo-grid-item, .expo-block, .expo-preview, .expo-teaser");

            // Пробуем селекторы для таблиц и списков
            System.out.println("\n[DEBUG_LOG] Пробуем селекторы для таблиц и списков:");
            analyzeSelectors(doc, "table tr, ul li, .list-item, .grid-item");
            ...
            // Анализируем все ссылки на странице
            System.out.println("\n[DEBUG_LOG] Анализ всех ссылок на странице:");
            analyzeAllLinks(doc);
    ...
    /**
     * Анализирует все ссылки на странице
     */
    private static void analyzeAllLinks(Document doc) {
        // Ищем все ссылки на странице
        Elements allLinks = doc.select("a");
        System.out.println("[DEBUG_LOG] Всего ссылок на странице: " + allLinks.size());

        // Ищем ссылки, содержащие ключевые слова, связанные с выставками
        ...
        for (int i = 0; i < Math.min(10, expoLinks.size()); i++) {
            Element link = expoLinks.get(i);
            System.out.println("[DEBUG_LOG] Ссылка #" + (i+1) + ": " + link.text() + " - " + link.attr("href"));

            // Проверяем родительский элемент ссылки
            Element parent = link.parent();
            System.out.println("[DEBUG_LOG] - Родительский элемент: " + parent.tagName() + " - классы: " + parent.className());
        }

        // Ищем ссылки, ведущие на страницы выставок
        Elements expoPageLinks = doc.select("a[href*=expo], a[href*=exhibition], a[href*=event]");
        System.out.println("\n[DEBUG_LOG] Найдено ссылок на страницы выставок: " + expoPageLinks.size());

        for (int i = 0; i < Math.min(10, expoPageLinks.size()); i++) {
            Element link = expoPageLinks.get(i);
            System.out.println("[DEBUG_LOG] Ссылка на выставку #" + (i+1) + ": " + link.text() + " - " + link.attr("href"));
        }
    }
